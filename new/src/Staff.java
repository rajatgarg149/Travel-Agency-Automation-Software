...
public class Staff {
	...
	public 
	 ...
	 static void AddModel(String car_model , int car_price , int new_FareChargePerKm , int new_FareChargePerHr, int new_Mileage , app window)
	 {
		 ///AddModel
		 int temp;
		 for(temp=0;temp<window.a_model;temp++)
		 {
			 if(window._Model.get(temp).getCarModel().equals(car_model))
			 {
		 ...
		 int response = JOptionPane.showConfirmDialog(null, "Are You Sure You Want To Add This Model", "Confirm",
		    	...
		    	window._Model.add(new Model());
		    	window._Model.get(window.a_model).setCarModel(car_model);
		    	window._Model.get(window.a_model).setCarPrice(car_price);
		    	...
		    	window._Model.get(window.a_model).setMileage(new_Mileage);
		    	window.a_model++;
	 }
